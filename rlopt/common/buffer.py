...
from rlopt.common.utils import split_and_pad_trajectories
    ...
    def compute_returns_and_advantage(
        ...
        for step in reversed(range(self.buffer_size)):
            if step == self.buffer_size - 1:
                next_non_terminal = 1.0 - dones.type(th.float32)  # .type(th.float32)
...
class RecurrentRolloutBuffer(RolloutBuffer):
    ...
    def get(
        ...
        split_index = th.randint(
            0, self.buffer_size * self.n_envs, size=(1,), device=self.device
        ).item()
...
class RLOptDictRecurrentReplayBuffer(ABC):
    ...
    def compute_returns_and_advantage(
        ...
        for step in reversed(range(self.buffer_size)):  # type: ignore[arg-type]
            if step == self.buffer_size - 1:
                next_non_terminal = (
                    ~dones if dones.dtype == th.bool else 1.0 - dones.type(th.float32)
                )  # .type(th.float32)
                next_values = last_values
            else:
                next_non_terminal = 1.0 - self.episode_starts[step + 1]
