...
class BilinearSR(ABC, nn.Module):
    ...
    - ``identity``: phi(s, a) = a. The action IS the representation, so the
      ...
      log-weight of one expert field ``E_k(s, s', t)``, and
      ``z_mix = a z_1 + b z_2`` samples the tempered product ``p_1^a p_2^b``
      for ANY a, b, not only a + b = 1.
    """
    ...
    def __init__(
        ...
        if self.phi_parameterization not in {
            ...
            msg = (
                ...
                "'affine', or 'identity' (aliases: 'legacy', 'f_matrix'), got "
        ...
        elif self.phi_parameterization == "affine":
...
class DiffSRBilinear(BilinearSR):
    ...
    def __init__(
        ...
        if self.mu_conditioning not in {"next", "pair"}:
            msg = (
                "mu_conditioning must be 'next' (mu(s', t)) or 'pair' "
                f"(mu(s, s', t)), got {mu_conditioning!r}."
            )
            raise ValueError(msg)
