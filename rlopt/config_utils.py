...
def load_env_config(
    env_name: str, config_dir: str | Path = "configs/env"
) -> dict[str, Any]:
...
def apply_env_config(rlopt_config, env_config: dict[str, Any]) -> None:
    ...
    if "network" in env_config:
        ...
        if "hidden_dims" in net:
            ...
            if (
                hasattr(rlopt_config, "q_function")
                and rlopt_config.q_function is not None
            ):
...
def get_env_type(env_name: str) -> str:
    ...
    if any(
        x in env_lower for x in ["halfcheetah", "walker", "hopper", "ant", "humanoid"]
    ):
        return "mujoco"
...
def next_obs_key(key: ObsKey) -> tuple[str, ...]:
    ...
    key = cast(ObsKey, normalize_batch_key(key))
    if isinstance(key, tuple):
        return ("next", *key)
    return ("next", key)
